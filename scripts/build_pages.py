...
js="let enginePromise;\nfunction getEngine(){if(!enginePromise)enginePromise=fetch('./knowledge.json').then(r=>{if(!r.ok)throw Error('Source library failed to load. Please reload.');return r.json()}).then(data=>new KilsaranSearch(data)).catch(e=>{enginePromise=null;throw e});return enginePromise;}\n"+js
...
assert start in js
js=js.replace(start,"const data=(await getEngine()).answer(q,previous,category);")
