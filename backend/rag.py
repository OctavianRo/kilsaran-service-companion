...
def answer_question(provider,store,model,question,history,category='All products'):
    ...
    for hit in hits.get('data',[]):
        ...
        parsed=urlparse(url)
        if parsed.scheme!='https' or parsed.hostname not in {'kilsaran.ie','www.kilsaran.ie'} or parsed.username:continue
    ...
    response=provider.post('responses',{'model':model,'store':False,'instructions':INSTRUCTIONS,'input':json.dumps({'question':question,'conversation':history[-6:],'evidence':sources}), 'max_output_tokens':1600,'text':{'format':{'type':'json_schema','name':'kilsaran_answer','strict':True,'schema':SCHEMA}}})
